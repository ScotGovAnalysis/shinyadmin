...
apps <-
  ...
  mutate(
    org_group = org_group_to_factor(org_group),
    sg_app = str_starts(org_group, "Scottish Government"),
...
apps %>% 
  filter(str_starts(org_group, "Scottish Government")) %>%
...
apps %>% 
  filter(org_group == "Public Health Scotland") %>%
  select(url, org, updated_time, total_hours, form_completed_time) %>%
  arrange(url) %>%
  app_table()
```
...
#| title: Inactive
inactive %>% 
  arrange(org_group, org != "Scottish Government", url) %>%
  select(url, org, status, updated_time, total_hours, form_completed_time) %>%
  app_table()
```
